    ...
    try {
        ...
        if (data.valid) {
            ...
            const busDetails = data.ride_type
                ...
                : '';
            const hasDetails = !!(data.ticket_type_name || data.order_number || data.attendee_first_name || data.attendee_last_name || data.ride_type);
            const attendeeLine = (data.attendee_first_name || data.attendee_last_name)
                ? `${data.attendee_first_name || ''} ${data.attendee_last_name || ''}<br>`
                : '';
            const campingDetails = hasDetails ? `<br>Camping : ${data.is_camping ? 'Oui' : 'Non'}` : '';
            resultEl.classList.add('invalid');
            resultEl.innerHTML = `
                <div class="result-icon">❌</div>
